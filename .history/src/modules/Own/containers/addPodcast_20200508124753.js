...
export default class addPodcast extends React.Component {
 ...
 async bucleAddSong(element) {
    ...
    await NetworkService.addCancionAlbum(element.nombre,element.URI,this.state.idpodcast, this.state.user.correo)//this.state.user.correo
  ...
  crearAlbum = async () => {
    if (this.state.nombrePodcast != ""){
      ...
      this.state.cancionesPodcast.forEach(element => this.bucleAddCapitulo(element) );
    }
