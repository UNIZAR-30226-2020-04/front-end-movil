		...
		setInterval(() => {
			this.setState(() => {
				this.retrievePlaylist().then( res => {
					...
					PLAYLIST_OLD.map(element => JSON.stringify(element))
					PLAYLIST_NUEVA.map(element => JSON.stringify(element))

					.map(cancion =><Text style={styles.text}>  {cancion.nombre} </Text> )}
					
					if(arraysAreEqual(PLAYLIST_OLD,PLAYLIST_NUEVA)){//PLAYLIST_OLD.toString() != PLAYLIST_NUEVA.toString()
