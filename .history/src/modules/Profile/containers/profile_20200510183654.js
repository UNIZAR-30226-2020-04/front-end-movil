...
export default class Profile extends Component{
  ...
  async componentDidMount(){
    ...
    await NetworkService.fetchAlbums(this.state).then(res => {this.setState({albums: res}); console.log("ALBUMES: ",res)});
            ...
            <View style={{height: 200, marginTop: 20}}>
              <ScrollView horizontal={true}>
                ...
                {this.state.albums.map(
                  element => 
                  if(){}
                  <Element type='album' props={element.idAlbum.l_id} image = {{uri: }}album_name="Venom" artist="cualquiera"></Element>               
                )}
              </ScrollView>
            </View>
          </View>
