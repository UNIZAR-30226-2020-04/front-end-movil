...
export default class Main extends Component{
  ...
  storeUser = async () => {
    try {
      ...
      console.log("Guardado this.user: ", this.state.user.state)
    } catch (error) {
  ...
  updateUserData = async () =>{
    ...
    if(this.state.user.pass != "" && this.state.user.newPass != ""){
      ...
      <ImageBackground source={require('../../../Wallpapers/fondo.jpg')} style={styles.container}>
        <ScrollView style={styles.screen}>
          ...
          <View style={{alignItems: 'center'}}>
              <Text style={[styles.text, {fontSize: 24,fontWeight: '600', marginTop: 10}]}>{this.state.user.name}</Text>
