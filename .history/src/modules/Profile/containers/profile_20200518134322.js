...
export default class Profile extends Component{
  ...
  getPodcastsDB = async () => {
    try {
      console.log("STATE.USER",this.state.user)
      await NetworkService.fetchPodcasts(this.state.user).then(res => {this.setState({podasts: res, loadedPodcasts:true});console.log("GETPodcasts RES:", res);console.log("GETPodcasts Podcasts:", this.state.podasts)});
    } catch (error) {
            ...
            <View style={{height: 200, marginTop: 20}}>
              ...
              {console.log("this.state.podcasts: ", this.state.podcasts)}
                {this.state.podcasts.map(
                  element => 
                    <Element type='podcast' paramId={element.idAlbum.l_id} image = {{ uri: element.foto==null ? DEFAULT_URI : element.foto }} name={element.nombre} artist={element.idAlbum.u} props={this.props}></Element>
                  )
                }   
              </ScrollView>
            </View>
